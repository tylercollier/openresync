...
// Batch type should be 'upsert' or 'purge'.
...
async function deleteFilesForMlsResource(sourceFilesForMlsResource) {
...
async function deleteSourceFilesForBatch(userConfig, mlsSourceName, batchType, batchId, eventEmitter) {
  const filesPerMlsResource = await getSourceFiles(userConfig, mlsSourceName)
  ...
  for (const sourceFilesForMlsResource of sourceFilesForBatch) {
    await deleteFilesForMlsResource(sourceFilesForMlsResource)
  }
}
...
function getOldestBatchId(filesPerMlsResource, batchType) {
  ...
  for (const filesArray of filesPerMlsResource) {
    ...
    for (const filePath of filesArray) {
      ...
      const batchId = fileName.match(regex)
...
function getPrimaryKeyField(mlsResource, indexes) {
