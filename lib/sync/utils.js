...
async function getSourceFiles(userConfig, mlsSourceName) {
  const filesPerMlsResource = await Promise.all(userConfig.sources[mlsSourceName].mlsResources.map(mlsResourceName => getMlsResourceDirFiles(mlsSourceName, mlsResourceName)))
