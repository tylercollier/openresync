...
module.exports = function(mlsSourceName, configBundle, internalEventEmitter, loggerArg) {
  ...
  async function resumeSync(batchType, metadata) {
    ...
    try {
      ...
      emitToAll(eventType === 'sync' ? ORS_SYNC_START : ORS_RECONCILE_START, { mlsSourceName, batchId })
      ...
      for (let i = startIndex; i < mlsResources.length; i++) {
        ...
        emitToAll(eventType === 'sync' ? ORS_SYNC_RESOURCE_START : ORS_RECONCILE_RESOURCE_START, { mlsResourceObj })
        ...
        await flushInternalConfig()
        emitToAll(eventType === 'sync' ? ORS_SYNC_RESOURCE_DONE : ORS_RECONCILE_RESOURCE_DONE, { mlsResourceObj })
      ...
      await flushInternalConfig()
      emitToAll(eventType === 'sync' ? ORS_SYNC_DONE : ORS_RECONCILE_DONE, { mlsSourceName, batchId })
      logger.info(`Done processing ${eventType} data for resources`)
    } catch (error) {
      emitToAll(eventType === 'sync' ? ORS_SYNC_ERROR : ORS_RECONCILE_ERROR, error)
      throw error
    }
  }
