...
module.exports = function(mlsSourceName, configBundle, eventEmitter, loggerArg) {
  ...
  async function resumeSync() {
    ...
    for (let i = startIndex; i < userConfig.sources[mlsSourceName].mlsResources.length; i++) {
      const mlsResourceObj = userConfig.sources[mlsSourceName].mlsResources[i]
  ...
  async function processSync({ mlsResourceObj, dataFilePath, destinationsStatus, fileNum, fileCount }) {
    const mlsResourceName = mlsResourceObj.name
