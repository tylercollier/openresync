...
module.exports = function(mlsSourceName, configBundle, eventEmitter, loggerArg) {
  ...
  async function resumeUpsert() {
    logger.info('Starting or resuming upsert process')
    ...
    if (processUpsertBatch) {
      logger.debug('Found existing process upsert batch data')
      ...
      batchId = getOldestBatchId(sourceFiles, 'upsert')
      ...
      if (downloadUpsertBatch) {
        ...
        if (batchId === downloadBatchId) {
          throw new Error(`Refusing to process upsert batch ${batchId} because it has not finished downloading`)
        }
      }
    ...
    logger.debug({ batchTimestamp: processUpsertBatch.batchTimestamp, batchId }, 'Timestamp for process upsert batch')
    ...
    logger.info('Start processing upsert data for resources')
    for (let i = startIndex; i < userConfig.sources[mlsSourceName].mlsResources.length; i++) {
      ...
      logger.info({ resource: mlsResource }, 'Start processing upsert data for resource')
      ...
      logger.info({ resource: mlsResource, filesCount: filesForMlsResource.length }, 'Processing upsert files')
      for (let i = 0; i < filesForMlsResource.length; i++) {
        const filePath = filesForMlsResource[i]
        logger.info({ resource: mlsResource, fileNum: i + 1, dataFilePath: filePath }, 'Processing upsert file')
        ...
        const success = await processUpsert({ mlsResource, dataFilePath: filePath, destinationsStatus })
        if (!success) {
          return
        }
      ...
      await flushInternalConfig()
      logger.info({ resource: mlsResource }, 'Done processing upsert data for resource')
    }
    _.unset(internalConfig, ['sources', mlsSourceName, 'processUpsertBatch'])
    await flushInternalConfig()
    logger.info('Done processing upsert data for resources')
  }

  async function processUpsert({ mlsResource, dataFilePath, destinationsStatus }) {
    ...
    for (let i = startIndex; i < destinations.length; i++) {
      ...
      if (destinationStatus.errorCount >= maxErrorCount) {
        logger.warn({ resource: mlsResource, dataFilePath, destination: destination.name }, `Destination has reached max error count of ${maxErrorCount}. Refusing to process upsert.`)
        return false
      }

      try {
        logger.info({ resource: mlsResource, dataFilePath, destination: destination.name }, 'Upserting to destination')
        ...
        await flushInternalConfig()
        logger.error({ err: e }, 'Error upserting')
        return false
      }
    }
    logger.info({ resource: mlsResource, dataFilePath }, 'Done upserting file data')
    logger.info({ resource: mlsResource, dataFilePath }, 'Deleting upsert file')
  ...
  async function resumePurge() {
    ...
    for (let i = startIndex; i < userConfig.sources[mlsSourceName].mlsResources.length; i++) {
      ...
      // We can't process the files one by one, like with upsert. Instead, we concat all the data
