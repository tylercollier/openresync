...
module.exports = function(mlsSourceName, configBundle, eventEmitter, loggerArg) {
      ...
      for (let i = startIndex; i < mlsResources.length; i++) {
        ...
        if (processSyncBatchMlsResource.currentFilePath) {
          if (filesForMlsResource.length && processSyncBatchMlsResource.currentFilePath !== filesForMlsResource[0]) {
            const e = new Error(`Existing process batch's currentFilePath (${processSyncBatchMlsResource.currentFilePath}) doesn't match next file to process (${filesForMlsResource[0]}`)
  ...
  async function getMissingIds(mlsResourceName, dataInMls, indexes) {
    const allMissingIdsArray = await Promise.all(destinations.map(async (destination, i) => {
      ...
      logger.trace({ resource: mlsResourceName, destination: destination.name }, 'Done getting missing IDs from destination for resource')
      return missingIdsForDestination
    }))
    let allMissingIds = allMissingIdsArray.flat()
