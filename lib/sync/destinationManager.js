...
module.exports = function(mlsSourceName, configBundle, eventEmitter, loggerArg) {
  ...
  async function resumeSync() {
    ...
    if (processSyncBatch) {
      ...
      processSyncBatch = {
        batchTimestamp,
        mlsResourcesStatus: userConfig.sources[mlsSourceName].mlsResources.map(mlsResourceObj => ({
    ...
    for (let i = startIndex; i < userConfig.sources[mlsSourceName].mlsResources.length; i++) {
      const mlsResourceName = userConfig.sources[mlsSourceName].mlsResources[i].name
  ...
  async function syncMetadata(metadataString) {
    ...
    await Promise.all(_.flatMap(mlsResources, mlsResourceObj => {
      ...
      return dataAdapters.map(dataAdapter => {
        return dataAdapter.syncStructure(metadata, mlsResourceObj.name, indexes)
      })
    }))
  }
