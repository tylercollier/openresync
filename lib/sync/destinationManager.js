...
module.exports = function(mlsSourceName, configBundle, eventEmitter, loggerArg) {
  ...
  async function resumeSync() {
    ...
    if (processSyncBatch) {
      ...
      processSyncBatch = {
        batchTimestamp,
        mlsResourcesStatus: userConfig.sources[mlsSourceName].mlsResources.map(mlsResourceName => ({
          name: mlsResourceName,
          done: false,
        })),
      }
    ...
    for (let i = startIndex; i < userConfig.sources[mlsSourceName].mlsResources.length; i++) {
      const mlsResourceName = userConfig.sources[mlsSourceName].mlsResources[i]
  ...
  async function processSync({ mlsResourceName, dataFilePath, destinationsStatus, fileNum, fileCount }) {
    ...
    for (let i = startIndex; i < destinations.length; i++) {
      ...
      try {
        ...
        if (recordsToSyncCount > 0) {
          ...
          await dataAdapter.syncData(mlsResourceName, mlsData.value)
        } else {
  ...
  async function syncMetadata(metadataString) {
    ...
    await Promise.all(_.flatMap(mlsResources, mlsResourceName => {
      const indexes = getIndexes(mlsResourceName)
      return dataAdapters.map(dataAdapter => {
        return dataAdapter.syncStructure(metadata, mlsResourceName, indexes)
      })
    }))
  }
