...
module.exports = function(mlsSourceName, configBundle, eventEmitter, loggerArg) {
  ...
  async function getMissingIds(mlsResourceName, dataInMls, indexes) {
    const allMissingIdsDataArray = await Promise.all(destinations.map(async (destination, i) => {
      ...
      // let filePath = pathLib.resolve(__dirname, `destination${i}.json`)
      // const fileData = await fsPromises.readFile(filePath, 'utf-8')
      // const missingIdsDataForDestination = JSON.parse(fileData)
      // await fsPromises.writeFile(filePath, JSON.stringify(missingIdsDataForDestination))
    ...
    const allMissingIdsArray = await Promise.all(destinations.map((destination, i) => {
      const dataAdapter = dataAdapters[i]
      ...
      return dataAdapter.computeMissingIds(mlsResourceName, dataInMls, dataInAdapter, indexes)
