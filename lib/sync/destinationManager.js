...
module.exports = function(mlsSourceName, configBundle, eventEmitter, loggerArg) {
    ...
    try {
      ...
      for (let i = startIndex; i < mlsResources.length; i++) {
        ...
        for (const filePath of filesForMlsResource) {
          ...
          idsToPurge = idsToPurge.concat(ids)
        }
        logger.trace({ resource: mlsResourceName }, 'Sorting IDs')
        if (!mlsSourceUserConfig.useOrderBy) {
          idsToPurge.sort()
        }
        const success = await processPurge({ mlsResourceObj, idsToPurge, destinationsStatus, getIndexes: platformAdapter.getIndexes })
        if (!success) {
          return
        }
  ...
  async function processPurge({ mlsResourceObj, idsToPurge, destinationsStatus, getIndexes }) {
    const mlsResourceName = mlsResourceObj.name
    ...
    for (let i = startIndex; i < destinations.length; i++) {
      ...
      try {
        ...
        if (idsToPurge.length) {
          ...
          logger.debug({ resource: mlsResourceName, destination: destination.name }, 'Successfully purged records')
        } else {
          if (!'debug') {
