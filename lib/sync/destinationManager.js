...
module.exports = function(mlsSourceName, configBundle, eventEmitter, loggerArg) {
  ...
  async function resumeSync(batchType, metadata) {
    const eventType = batchType === 'missing' ? 'reconcile' : batchType
    try {
      ...
      const configObjPath = ({
        addFields: 'processAddFieldsBatch',
        missing: 'processReconcileBatch',
        sync: 'processSyncBatch',
      })[batchType]
      ...
      if (processBatch) {
        ...
        // TODO: ensure process batch obj's mlsResourcesStatus matches current config.
      } else {
        ...
        if (eventType === 'sync') {
          ...
          const downloadSyncBatch = getBatch(mlsSourceInternalConfig, configObjPath)
    ...
    for (let i = startIndex; i >= 0 && i < destinations.length; i++) {
      ...
      try {
        ...
        if (recordsToSyncCount > 0) {
          const msg = eventType === 'addFields'
            ? 'Updating X records in destination'
          ...
          if (eventType === 'addFields') {
            await dataAdapter.updateData(mlsResourceObj, mlsData.value, metadata)
          } else {
