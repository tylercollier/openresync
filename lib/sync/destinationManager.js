...
module.exports = function(mlsSourceName, configBundle, eventEmitter) {
  ...
  async function resume() {
    ...
    let processBatch = _.get(internalConfig, ['sources', mlsSourceName, 'processBatch'])
    let batchId
    if (processBatch) {
      const batchTimestamp = moment.utc(processBatch.batchTimestamp)
      batchId = convertTimestampToBatchId(batchTimestamp)
      // TODO: ensure processBatch's mlsResourcesStatus matches current config.
    } else {
      ...
      const downloadBatch = getBatch(internalConfig, mlsSourceName, 'downloadBatch')
      if (downloadBatch) {
        const downloadBatchId = convertTimestampToBatchId(downloadBatch.batchTimestamp)
    ...
    const sourceFilesForBatch = getSourceFilesForBatch(sourceFiles, batchId, 'batch')
    ...
    for (let i = startIndex; i < userConfig.sources[mlsSourceName].mlsResources.length; i++) {
      ...
      if (processBatchMlsResource.currentFilePath) {
        if (filesForMlsResource.length && processBatchMlsResource.currentFilePath !== filesForMlsResource[0]) {
          throw new Error(`Existing process batch's currentFilePath (${processBatchMlsResource.currentFilePath}) doesn't match next file to process (${filesForMlsResource[0]}`)
        }
      }
