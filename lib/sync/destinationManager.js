...
module.exports = function(mlsSourceName, configBundle, internalEventEmitter, loggerArg) {
  let platformAdapter
  ...
  const userEventEmitter = mlsSourceUserConfig.eventEmitter
  const emitToAll = makeMultiEmitter(internalEventEmitter, userEventEmitter)
  ...
  async function resumeSync(batchType, metadata) {
    ...
    try {
      ...
      // Throughout this function, I'll use this eventName temp variable, to make sure we explicitly reference the
      // event name constants.
      const eventName = eventType === 'sync' ? ORS_SYNC_START : ORS_RECONCILE_START
      emitToAll(eventName, { mlsSourceName, batchId })
      ...
      for (let i = startIndex; i < mlsResources.length; i++) {
        ...
        const eventName = eventType === 'sync' ? ORS_SYNC_RESOURCE_START : ORS_RECONCILE_RESOURCE_START
        emitToAll(eventName, { mlsResourceObj })
        ...
        await flushInternalConfig()
        const eventName = eventType === 'sync' ? ORS_SYNC_RESOURCE_DONE : ORS_RECONCILE_RESOURCE_DONE
        emitToAll(eventName, { mlsResourceObj })
      ...
      await flushInternalConfig()
      const eventName = eventType === 'sync' ? ORS_SYNC_DONE : ORS_RECONCILE_DONE
      emitToAll(eventName, { mlsSourceName, batchId })
      logger.info(`Done processing ${eventType} data for resources`)
    } catch (error) {
      const eventName = eventType === 'sync' ? ORS_SYNC_ERROR : ORS_RECONCILE_ERROR
      emitToAll(eventName, error)
      throw error
    }
  }
  ...
  async function resumePurge() {
    try {
      ...
      emitToAll(ORS_PURGE_START, { mlsSourceName, batchId })
      ...
      for (let i = startIndex; i < mlsResources.length; i++) {
        ...
        emitToAll(ORS_PURGE_RESOURCE_START, { mlsResourceObj })
        const filesForMlsResource = sourceFilesForBatch[i]
        ...
        const success = await processPurge({ mlsResourceObj, idsInMls, destinationsStatus, getIndexes: platformAdapter.getIndexes })
        if (!success) {
          return
        }
        ...
        emitToAll(ORS_PURGE_RESOURCE_DONE, { mlsResourceObj })
        await deleteFilesForMlsResource(mlsResourceName, filesForMlsResource, logger.child({ resource: mlsResourceName }))
      }
      _.unset(mlsSourceInternalConfig, ['processPurgeBatch'])
      await flushInternalConfig()
      emitToAll(ORS_PURGE_DONE, { mlsSourceName, batchId })
      logger.info('Done processing purge data for resources')
    } catch (error) {
      emitToAll(ORS_PURGE_ERROR, error)
      throw error
    }
  }
  ...
  async function processPurge({ mlsResourceObj, idsInMls, destinationsStatus, getIndexes }) {
    ...
    for (let i = startIndex; i < destinations.length; i++) {
      ...
      try {
        ...
        const mlsIdsInDestination = await dataAdapter.getAllMlsIds(mlsResourceName, mlsIndexes)
        const mlsIdsToPurge = _.difference(mlsIdsInDestination, idsInMls)
        if (mlsIdsToPurge.length) {
          logger.info({ resource: mlsResourceName, destination: destination.name, recordsCount: mlsIdsToPurge.length }, 'Purging X records')
          ...
          if (mlsIdsToPurge.length > maxAllowedPurgeCount) {
            throw new Error(`Refusing to purge ${mlsIdsToPurge.length} records because count is greater than max allowed purge count of ${maxAllowedPurgeCount}`)
          }
          if (!'debug') {
            throw new Error('test purge error')
          }
          await dataAdapter.purge(mlsResourceObj, mlsIdsToPurge, getIndexes)
          logger.debug({ resource: mlsResourceName, destination: destination.name, recordsCount: mlsIdsInDestination.length - mlsIdsToPurge.length }, 'Successfully purged records. Net is X records.')
        } else {
          if (!'debug') {
        ...
        destinationStatus.done = true
        await flushInternalConfig()
        emitToAll(ORS_PURGE_DESTINATION_PAGE, { destination, idsPurged: mlsIdsToPurge })
      } catch (e) {
        const unpackedError = unpackErrorForSerialization(e)
        destinationStatus.error = unpackedError
        destinationStatus.errorCount++
