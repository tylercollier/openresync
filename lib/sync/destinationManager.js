...
module.exports = function(mlsSourceName, configBundle, eventEmitter, loggerArg) {
  ...
  async function resumeSync() {
    ...
    for (let i = startIndex; i < userConfig.sources[mlsSourceName].mlsResources.length; i++) {
      ...
      for (let i = 0; i < filesForMlsResource.length; i++) {
        ...
        const fileNum = i + 1
        logger.info({ resource: mlsResource, fileNum, dataFilePath: filePath }, 'Processing sync file')
  ...
  async function processSync({ mlsResource, dataFilePath, destinationsStatus, fileNum, fileCount }) {
    ...
    for (let i = startIndex; i < destinations.length; i++) {
      ...
      try {
        ...
        if (fileNum === 1) {
          existingRecordsInDestinationCount = await dataAdapter.getCount(mlsResource)
          logger.debug({ resource: mlsResource, destination: destination.name, recordsCount: existingRecordsInDestinationCount }, 'Starting with X records in destination')
        }
        if (recordsToSyncCount > 0) {
          ...
          logger.info({ resource: mlsResource, destination: destination.name }, 'File had 0 records. Skipping sync.')
        }
        destinationStatus.done = true
        await flushInternalConfig()
        if (fileNum === fileCount) {
          const recordsInDestinationCount = await dataAdapter.getCount(mlsResource)
          logger.debug({ resource: mlsResource, destination: destination.name, recordsCount: recordsInDestinationCount }, 'Ended with X records in destination')
        }
      } catch (e) {
    ...
    for (let i = startIndex; i < destinations.length; i++) {
      ...
      try {
        ...
        if (idsToPurge.length) {
          logger.info({ resource: mlsResource, destination: destination.name, recordsCount: idsToPurge.length }, 'Purging X records')
          await dataAdapter.purge(mlsResource, idsToPurge, mlsIndexes)
          logger.debug({ resource: mlsResource, destination: destination.name, recordsCount: idsInDestination.length - idsToPurge.length }, 'Successfully purged records. Net is X records.')
        } else {
