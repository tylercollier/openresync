...
module.exports = function(mlsSourceName, configBundle, eventEmitter) {
  ...
  async function resumePurge() {
    ...
    for (let i = startIndex; i < userConfig.sources[mlsSourceName].mlsResources.length; i++) {
      ...
      await deleteFilesForMlsResource(filesForMlsResource)
    }
  ...
  async function processPurge({ mlsResource, idsInMls, destinationsStatus, mlsIndexes }) {
    log(`Start purging MLS resource: ${mlsResource}`)
    const startIndex = destinationsStatus.findIndex(x => !x.done)
    for (let i = startIndex; i < destinations.length; i++) {
      const destination = destinations[i]
      const dataAdapter = dataAdapters[i]
      const destinationStatus = destinationsStatus[i]
      if (destinationStatus.errorCount >= maxErrorCount) {
        log(`Destination ${destinationStatus.name} has reached max error count of ${maxErrorCount}. Refusing to process purge. Stopping.`)
        return false
      }

      try {
        log(`Getting existing IDs for MLS resource: ${mlsResource}, destination ${destination.name}`)
        ...
        if (idsToPurge.length) {
          log(`Purging data for MLS resource: ${mlsResource}, destination ${destination.name}`)
          ...
          log(`No IDs to purge for MLS resource: ${mlsResource}, destination ${destination.name}`)
        }
        destinationStatus.done = true
        await flushInternalConfig()
      } catch (e) {
        ...
        destinationStatus.errorCount++
        await flushInternalConfig()
        log(`Error purging data: ${JSON.stringify(unpackedError)}`)
        return false
      }
    }
    log(`Done purging MLS resource: ${mlsResource}`)
    return true
  }
