...
module.exports = function(mlsSourceName, configBundle, eventEmitter, loggerArg) {
  ...
  async function processUpsert({ mlsResource, dataFilePath, destinationsStatus }) {
    logger.info({ resource: mlsResource, dataFilePath }, 'Process upsert')
    ...
    for (let i = startIndex; i < destinations.length; i++) {
      ...
      if (destinationStatus.errorCount >= maxErrorCount) {
        logger.info({ resource: mlsResource, dataFilePath, destination: destination.name }, `Destination has reached max error count of ${maxErrorCount}. Refusing to process upsert.`)
        return false
      }
    ...
    logger.info({ resource: mlsResource, dataFilePath }, 'Done upserting')
    logger.info({ resource: mlsResource, dataFilePath }, 'Deleting')
  ...
  async function processPurge({ mlsResource, idsInMls, destinationsStatus, mlsIndexes }) {
    ...
    for (let i = startIndex; i < destinations.length; i++) {
      ...
      const destinationStatus = destinationsStatus[i]
      if (destinationStatus.errorCount >= maxErrorCount) {
        logger.error({ resource: mlsResource, destination: destination.name }, `Destination has reached max error count of ${maxErrorCount}. Refusing to process purge.`)
        return false
      }

      try {
        logger.info({ resource: mlsResource, destination: destination.name }, 'Getting existing IDs')
  ...
  async function syncMetadata(metadataString) {
    ...
    const metadata = await parser.parseStringPromise(metadataString)
      .catch(catcher('parse metadata', { logger }))
