...
async function getTimestampsFromDownloadedFiles(mlsSourceName, mlsResource, indexes) {
...
async function go(mlsSourceName) {
  ...
  for (const mlsResource of mlsResources) {
    const indexes = getIndexesToAdd(mlsResource)
    let timestamps = await getTimestampsFromDownloadedFiles(mlsSourceName, mlsResources, indexes)
    if (!timestamps) {
    ...
    // TODO: The 'gt' here (greater than) should really be gte unless we are certain that the previous
    // download batch completed. So, figure out a way to determine if the previous download batch completed.
    // It seems like a simple way would be to just seem if there is a downloadBatch. If not, use gt, otherwise use gte.
