...
const { catcher: catcherUtil } = require('./utils')
...
async function go(mlsSourceName) {
  ...
  for (const mlsResource of mlsResources) {
    ...
    await dataAdapter.syncStructure(metadata, mlsResource, indexes, platformAdapter, platformDataAdapter)
  ...
  for (const mlsResource of mlsResources) {
    ...
    while (url) {
      ...
      await dataAdapter.syncData(mlsResource, data, platformAdapter)
