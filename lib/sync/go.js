...
async function ensureAuthIsNotExpired(auth, userConfig, internalConfig, mlsSourceName) {
  if (!auth.expiresAt || auth.expiresAt < Date.now()) {
    return await fetchAuth(userConfig, internalConfig, mlsSourceName)
  }
  return auth
}
...
async function go(mlsSourceName) {
  ...
  if (metadataPath) {
    ...
    auth = await ensureAuthIsNotExpired(auth, userConfig, internalConfig, mlsSourceName)
