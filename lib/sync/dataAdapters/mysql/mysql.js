...
module.exports = ({ destinationConfig }) => {
  ...
  function transform(mlsResourceName, record, metadata, recordIndex, parentTransformedMlsData) {
    return userTransform
      ? userTransform(mlsResourceName, record, metadata, recordIndex, parentTransformedMlsData)
      : record
  }
  ...
  async function syncData(
    ...
    const tableName = makeTableName(mlsResourceObj.name)
    const indexes = platformAdapter.getIndexes(mlsResourceObj.name)
    // Looking at the first record to get the field names seems easy/effective, but should we look at the metadata?
    ...
    // MLS Grid violates their own metadata with field lengths for certain fields. This is causing the insert statements
    // to fail. Let's find out which fields they are, so we can either increase their length in MySQL, or report them
    // to MLS Grid. For the moment, we'll just truncate them.
    if (mlsResourceObj.name === 'Property') {
      const stringFields = metadata['edmx:Edmx']['edmx:DataServices'][0].Schema[0].EntityType[0].Property.filter(x => {
        return x.$.Type === 'Edm.String'
      })
      const stringFieldNames = stringFields.map(x => x.$.Name)
      const problematicFields = {}
      let numberOfProblematicRecords = 0
      mlsData.forEach(record => {
        let foundProblemForThisRecord = false
        for (const field of stringFields) {
          const fieldName = field.$.Name
          const maxLength = parseInt(field.$.MaxLength, 10)
          const val = record[fieldName]
          if (val && val.length > maxLength) {
            if (!foundProblemForThisRecord) {
              foundProblemForThisRecord = true
              numberOfProblematicRecords++
            }
            record[fieldName] = val.slice(0, maxLength)
            const previousMaxLengthFound = problematicFields[fieldName]
            if (!previousMaxLengthFound) {
              problematicFields[fieldName] = val.length
            } else {
              if (val.length > previousMaxLengthFound) {
                problematicFields[fieldName] = val.length
              }
            }
          }
      ...
      const thisIsMyBreakpointLine = true
    }
    ...
    const transformedMlsData = mlsData.map(x => {
      ...
      return transform(mlsResourceObj.name, val, metadata, transformedParentRecord)
    })
    ...
    const values = transformedMlsData.reduce((a, v) => {
      a.push(...fieldNames.map(fieldName => {
        if (typeof v[fieldName] === 'object' && v[fieldName] !== null) {
          return JSON.stringify(v[fieldName])
        }
  ...
  async function purge(mlsResourceObj, idsToPurge, getIndexes) {
    ...
    const officialFieldName = getPrimaryKeyField(mlsResourceName, indexes)
    const userFieldName = makeFieldName(mlsResourceName, officialFieldName)
    const modifiedIdsToPurge = idsToPurge.map(x => makePrimaryKeyValueForDestination(mlsResourceName, x))
  ...
  function computeMissingIds(mlsResourceName, dataInMls, dataInAdapter, indexes) {
    ...
    const modifiedDataInMls = _.clone(dataInMls)
