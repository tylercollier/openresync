...
module.exports = ({ destinationConfig }) => {
  ...
  const userSelectForSyncStructure = destinationConfig.selectForSyncStructure
  ...
  async function effectTable(mlsResourceObj, tableNames, entityTypes) {
    ...
    const select = selectForSyncStructure(mlsResourceObj)
    if (tableNames.includes(tableName)) {
      await syncTableFields(mlsResourceObj.name, entityType, indexes, select)
    } else {
      await createTable(mlsResourceObj.name, entityType, indexes, select)
      await createIndexes(tableName, indexes, mlsResourceObj.name)
    }
  }

  async function createTable(mlsResourceName, entityType, indexes, select) {
    const fieldsString = entityType.Property
      .filter(property => shouldIncludeField(property.$.Name, indexes, platformAdapter.shouldIncludeMetadataField, select))
      .map(x => buildColumnString(mlsResourceName, x)).join(", \n")
  ...
  async function syncTableFields(mlsResourceName, entityType, indexes, select) {
    const tableName = makeTableName(mlsResourceObjName)
    ...
    const metadataFieldNamesObj = _.reduce(entityType.Property.map(x => makeFieldName(mlsResourceObjName, x.$.Name)), (a, v) => {
      a[v] = true
      return a
    }, {})
    ...
    for (const metadataProperty of entityType.Property) {
      const tableFieldName = makeFieldName(mlsResourceObjName, metadataProperty.$.Name)
      if (!(tableFieldName in tableFieldNamesObj)) {
        if (shouldIncludeField(metadataProperty.$.Name, indexes, platformAdapter.shouldIncludeMetadataField, select)) {
  ...
  function selectForSyncStructure(mlsResourceObj) {
    return userSelectForSyncStructure ? userSelectForSyncStructure(mlsResourceObj) : mlsResourceObj.select
  }
  ...
  async function updateData(
    ...
    const transformedMlsData = mlsData.map(x => {
      ...
      // fields that don't exist. So we need to use it in combo with mapKeys()
      const val = _.pick(_.mapKeys(x, (value, f) => makeFieldName(mlsResourceObj.name, f)), fieldNames)
      const transformedRecord = transform(mlsResourceObj.name, val, metadata, undefined, cache)
      // I'm probably doing something wrong, but Knex doesn't seem to handle array values properly. For example, if the
      // transformed record has these values: { ListingId: 'abc123', MyColumn: ['hello', 'world'] }, it would make an
      // insert statement like this: insert into MyTable (ListingId, MyColumn) values ('abc123', 'hello', 'world'),
      // which produces the error: Column count doesn't match value count at row 1.
    ...
    return db.transaction(async trx => {
      const queries = transformedMlsData.map(x => {
        const officialFieldName = getPrimaryKeyField(mlsResourceObj.name, indexes)
        const userFieldName = makeFieldName(mlsResourceObj.name, officialFieldName)
        const id = makePrimaryKeyValueForDestination(mlsResourceObj.name, x[userFieldName])
        return trx(tableName)
          .where(userFieldName, id)
          .update(x)
      })
