...
module.exports = ({ destinationConfig }) => {
  ...
  async function syncTableFields(mlsResourceName, entityType, indexes, select) {
    ...
    const metadataFieldNamesObj = _.reduce(entityType.Property.map(x => makeFieldName(mlsResourceName, x.$.Name)), (a, v) => {
      a[v] = true
      return a
    }, {})
    ...
    for (const metadataProperty of entityType.Property) {
      const tableFieldName = makeFieldName(mlsResourceName, metadataProperty.$.Name)
