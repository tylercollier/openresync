...
module.exports = ({ destinationConfig }) => {
  ...
  const userTransformDocument = destinationConfig.transformDocument
  ...
  function transformDocument(mlsResourceName, record, metadata, cache) {
    if (userTransformDocument) {
      return userTransformDocument(mlsResourceName, record, metadata, cache)
    }
    return {}
  }
  ...
  async function syncData(mlsResourceObj, mlsData, metadata) {
    ...
    const cache = {}
    const documents = mlsData.map(x => transformDocument(mlsResourceObj.name, x, metadata, cache))
    for (const doc of documents) {
      await promisify(solrClient.add.bind(solrClient))(doc)
    }
