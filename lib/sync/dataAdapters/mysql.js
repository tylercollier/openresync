...
module.exports = ({ destinationConfig }) => {
  ...
  async function createTable(mlsResourceObj, entityType, indexes) {
    const fieldsString = entityType.Property
      .filter(property => shouldIncludeField(property, indexes, mlsResourceObj.select))
      .map(buildColumnString).join(", \n")
    const tableName = makeTableName(mlsResourceObj.name)
  ...
  async function syncTableFields(mlsResourceObj, entityType, indexes) {
    const tableName = makeTableName(mlsResourceObj)
    ...
    for (const metadataProperty of entityType.Property) {
      ...
      if (!(tableFieldName in tableFieldNamesObj)) {
        if (shouldIncludeField(metadataProperty, indexes, mlsResourceObj.select)) {
...
// property is from the RESO Web API XML metadata data dictionary
  ...
  function shouldIncludeField(property, indexes, select) {
    ...
    if (platformAdapter.shouldIncludeMetadataField(fieldName) === false) {
      return false
    }

    if (select) {
      return select.includes(fieldName)
    }
    return true
  }

  async function syncData(mlsResourceObj, mlsData, transaction) {
    if (!mlsData.length) {
      return
    }
    ...
    const tableName = makeTableName(mlsResourceObj.name)
    const fieldNames = Object.keys(mlsData[0])
      .filter(platformAdapter.shouldIncludeJsonField)
    ...
    if (transaction) {
      ...
      return db.transaction(async t => {
        ...
        for (const subMlsResourceObj of (mlsResourceObj.expand || [])) {
          ...
          await syncData(subMlsResourceObj, subData, t)
        }
      })
    }
  }
