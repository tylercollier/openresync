...
module.exports = (userConfig, mlsSourceName) => {
  ...
  async function syncStructure(metadata, mlsResource, indexes) {
    ...
    const entityTypes = schemas.find(x => x.$.Namespace === 'ODataService').EntityType
  ...
  function getDatabaseType(property) {
    ...
    // if (type.startsWith('CoreLogic.DataStandard.RESO.DD.Enums') || type.startsWith('Collection(PropertyEnums') || type.startsWith('PropertyEnums')) {
    if (type.startsWith('CoreLogic.DataStandard.RESO.DD.Enums') || type.match(/Collection\(.*Enums\)/) || type.match(/.*Enums\./)) {
      return 'TEXT'
      ...
      return 'JSON'
    // } else if (type.startsWith('Collection(PropertyComplexTypes')) {
...
// property is from the RESO Web API XML metadata data dictionary
  ...
  async function syncData(mlsResource, mlsData) {
    ...
    const tableName = makeTableName(mlsResource)
    const fieldNames = Object.keys(mlsData[0]).filter(x => !x.startsWith('X_') && !x.startsWith('@odata'))// && x !== 'Media' && x !== 'ListingInputOriginalMedia')
