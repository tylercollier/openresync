...
module.exports = (userConfig, mlsSourceName) => {
  ...
  async function syncStructure(metadata, mlsResource, indexes) {
    ...
    // await db.raw('drop table if exists `Office`')
    // await db.raw('drop table if exists `CustomProperty`')
    // await db.raw('drop table if exists `OpenHouse`')
    // await db.raw('drop table if exists `PropertyRooms`')
    // await db.raw('drop table if exists `Media`')
