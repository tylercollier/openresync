...
module.exports = ({ destinationConfig }) => {
  ...
  function transform(mlsResourceName, record) {
    return userTransform ? userTransform(mlsResourceName, record) : record
  }
...
// property is from the RESO Web API XML metadata data dictionary
  ...
  async function syncData(mlsResourceObj, mlsData, transaction = null) {
    if (!mlsData.length) {
      return
    }
    ...
    const transformedMlsData = mlsData.map(x => {
      ...
      return transform(mlsResourceObj.name, val)
    })
  ...
  async function getMissingIds(mlsResourceName, dataInMls, indexes) {
    ...
    const mysqlTimestampFieldNames = timestampFieldNames.map(makeFieldName)
    const fieldNamesSql = [userFieldName, ...mysqlTimestampFieldNames]
      .map(tickQuote)
      .join(', ')
    const sql = `SELECT ${fieldNamesSql} FROM ${tableName} ORDER BY ${officialFieldName}`
    const [rows] = await db.raw(sql)
