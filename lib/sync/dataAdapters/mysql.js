...
module.exports = (userConfig, mlsSourceName) => {
  const db = knex({
    client: 'mysql2',
    connection: userConfig.sources[mlsSourceName].mysql.connectionString,
    debug: true,
  })
  const userMakeTableName = userConfig.sources[mlsSourceName].makeTableName

  async function syncStructure(metadata, mlsResource, indexes, platformAdapter, platformDataAdapter) {
    ...
    if (userConfig.sources[mlsSourceName].mlsResources.includes(mlsResource)) {
      ...
      if (tableNames.includes(tableName)) {
        await syncTableFields(tableName, entityType, db, platformDataAdapter)
      } else {
        await createTable(tableName, entityType, platformDataAdapter)
  ...
  async function syncTableFields(tableName, entityType, db, platformDataAdapter) {
    ...
    for (const metadataProperty of entityType.Property) {
      ...
      if (!(tableFieldName in tableFieldNamesObj)) {
        if (shouldIncludeField(metadataProperty)) {
          const typeString = getDatabaseType(metadataProperty, platformDataAdapter)
  ...
  function getDatabaseType(property, platformDataAdapter) {
...
// property is from the RESO Web API XML metadata data dictionary
  function buildColumnString(property, platformDataAdapter) {
    const dbType = getDatabaseType(property, platformDataAdapter)
  ...
  async function createTable(tableName, entityType, platformDataAdapter) {
    const types = _.groupBy(entityType.Property, x => x.$.Type)
    let fieldsString = entityType.Property.filter(shouldIncludeField).map(x => buildColumnString(x, platformDataAdapter)).join(", \n")
  ...
  async function syncData(mlsResource, mlsData, platformAdapter) {
    if (!mlsData.length) {
      return
    }
