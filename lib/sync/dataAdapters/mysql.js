...
const { doesConfigIncludeMlsResourceName } = require('../utils/mlsResources')
...
module.exports = (userConfig, mlsSourceName, destinationConfig) => {
  ...
  async function syncStructure(metadata, mlsResourceName, indexes) {
    ...
    // await db.raw(`drop table if exists \`${makeTableName(mlsResourceName)}\``)
    // await db.raw(`truncate \`${makeTableName(mlsResourceName)}\``)
    ...
    if (doesConfigIncludeMlsResourceName(userConfig, mlsSourceName, mlsResourceName)) {
      const tableName = makeTableName(mlsResourceName)
      const entityType = entityTypes.find(x => x.$.Name === mlsResourceName)
