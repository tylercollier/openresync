...
const missingOrOldObjs = dataInMls.reduce((a, v, index) => {
  ...
  if (dbObj) {
    for (let i = 0; i < timestampFieldNames.length; i++) {
      ...
      if (v[timestampFieldNames[i]] === null && dbObj[mongoDBTimestampFieldNames[i]] === null) {
        continue
      }
      ...
      const dbTimestamp = dbObj[mongoDBTimestampFieldNames[i]]
      const equal = mlsTimestamp.getTime() === new Date(dbTimestamp).getTime()
      if (!equal) {
        a.push(v)
        break
      }
