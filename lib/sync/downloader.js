...
module.exports = function(mlsSourceName, configBundle, eventEmitter, loggerArg) {
  ...
  async function downloadPurgeData() {
    ...
    const topLevelMlsResources = userConfig.sources[mlsSourceName].mlsResources
    ...
    for (const mlsResourceObj of mlsResources) {
      ...
      let url = new URL(userConfig.sources[mlsSourceName].getPurgeEndpoint(mlsResourceObj, isExpandedMlsResource))
  ...
  async function downloadMlsResources() {
    ...
    for (const mlsResourceObj of mlsResources) {
      ...
      let url = userConfig.sources[mlsSourceName].getReplicationEndpoint(mlsResourceObj)
      ...
      if (mlsResourceObj.expand) {
        urlWithTimestamps.searchParams.set('$expand', mlsResourceObj.expand.map(x => x.fieldName).join(','))
      }
