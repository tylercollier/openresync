...
module.exports = function(mlsSourceName, configBundle, eventEmitter, loggerArg) {
  ...
  async function fetchWithProgressWithRetry({ url, mlsResourceName, auth }) {
    let mlsDataString
    let mlsData
    let loopCount = 0
    const maxLoops = 3
  ...
  async function downloadMlsResources() {
    ...
    for (const mlsResourceObj of mlsResources) {
      ...
      while (url) {
        ...
        auth = await ensureAuthIsNotExpired(auth)
        const { mlsData, mlsDataString } = await fetchWithProgressWithRetry({ url, mlsResourceName, auth })
