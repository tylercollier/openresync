...
// NOTE1: Why aren't we using $top with CRMLS? Because they allow different max records based on the resource. They
// claim they allow 100 for Property, but I got errors regarding a page size over 50 in certain occasions, and the
// easier solution temporarily is to just not specify it. But were I to refactor the solution, I'd change $top into a
// function in the platform adapter where it'd could decide the $top based on the resource.
...
module.exports = function(mlsSourceName, configBundle, eventEmitter, loggerArg) {
  ...
  function fmt(value) {
    // TODO: I'm using -07:00 here, but any offset, including Z, is currently ignored, and CRMLS local time is used.
    //   So really what's important is the subtract(7, 'hours'), and it'd need to be 8 hours when daylight time ends.
    //   And of course this only applies to CRMLS. It should really be moved into the platform adapter.
    const utcTime = moment.utc(value).subtract(7, 'hours').toISOString().replace('Z', '-07:00')
    return `cast('${utcTime}', Edm.DateTimeOffset)`
  }
  ...
  function makeUrl(mlsResourceObj, timestamps, batchTimestamp, top, batchAlreadyExisted, indexes) {
    ...
    const gtTimestampsString = _.map(timestamps, (value, key) => `${key} ${getOp(value, batchAlreadyExisted)} ` + fmt(value)).join(' or ')
    const ltBatchTimestampString = _.map(timestamps, (value, key) => {
      let s = ''
      s = `${key} le ` + fmt(batchTimestamp)
    ...
    // TODO: See NOTE1 at the top of this file for why we aren't using $top (with CRMLS).
