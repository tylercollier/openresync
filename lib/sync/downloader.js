...
module.exports = function(mlsSourceName, configBundle, eventEmitter, loggerArg) {
  ...
  async function downloadPurgeData() {
    ...
    let purgeStatusPerResource = _.get(mlsSourceInternalConfig, 'purgeStatusPerResource')
    if (!purgeStatusPerResource) {
      purgeStatusPerResource = mlsResources.reduce((a, v) => {
        a[v.name] = {
          lastPurgedAt: 0,
        }
        return a
      }, {})
      _.set(mlsSourceInternalConfig, 'purgeStatusPerResource', purgeStatusPerResource)
    }
    ...
    for (const mlsResourceObj of mlsResources) {
      ...
      const purgeStatusForResource = purgeStatusPerResource[mlsResourceName]
      const lastPurgedAt = purgeStatusForResource.lastPurgedAt
      platformAdapter.modifyUrlForPurge(url, mlsResourceObj, isExpandedMlsResource, lastPurgedAt)
      // Make a note of when the call to the platform is, and we'll save it to the internal config if the purge works.
      const newLastPurgedAt = moment.utc().unix()
      ...
      purgeStatusForResource.lastPurgedAt = newLastPurgedAt
      await flushInternalConfig()
