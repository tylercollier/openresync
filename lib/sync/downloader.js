...
module.exports = function(mlsSourceName, configBundle, eventEmitter, loggerArg) {
  ...
  function makeUrl(mlsResourceObj, timestamps, batchTimestamp, top, batchAlreadyExisted, indexes) {
    ...
    const ltBatchTimestampString = _.map(timestamps, (value, key) => {
      let s = ''
      s = `${key} le ` + batchTimestamp.toISOString()
      // We want the timestamp to be less than our batch timestamp, but also include null values
  ...
  // We return the parsed data, or undefined if it can't be parsed. If we return falsy, we expect our caller to
  // increment its invalid-server-response counter. This is not a good approach if the JSON data is ever expected to
  // legitimately be falsy.
  async function handleParseResponse(mlsDataString, invalidServerResponseCount, mlsResourceName, batchTimestamp,
    mlsResourceDir) {
    let mlsData
    try {
      mlsData = JSON.parse(mlsDataString)
    } catch (error) {
      if (invalidServerResponseCount >= 3) {
        catcher('download sync')(error, {resource: mlsResourceName})
      }
      const invalidResponseFileName = makeBatchFileName('invalid_response',
        convertTimestampToBatchId(batchTimestamp))
      const invalidResponseFilePath = pathLib.resolve(mlsResourceDir, invalidResponseFileName)
      const msg = 'Invalid response from server (not JSON). Writing to file for investigation, and retrying'
      logger.info({ resource: mlsResourceName, invalidResponseFilePath }, msg)
      await fsPromises.writeFile(invalidResponseFilePath, mlsDataString)
    }
    return mlsData;
  }
  ...
  async function downloadMlsResources() {
    ...
    for (const mlsResourceObj of mlsResources) {
      ...
      let url = makeUrl(mlsResourceObj, timestamps, batchTimestamp, top, batchAlreadyExisted, indexes)
      ...
      let invalidServerResponseCount = 0
      while (url) {
        downloadPageCount++
        ...
        const mlsData = await handleParseResponse(mlsDataString, invalidServerResponseCount, mlsResourceName,
          batchTimestamp, mlsResourceDir)
        if (!mlsData) {
          invalidServerResponseCount++
          continue
        }
        invalidServerResponseCount = 0
      ...
      while (url) {
        ...
        const mlsDataString = await fetchWithProgress({
          ...
          .catch(error => {
            // logger.error({ err: error.response, resource: mlsResourceName })
            // throw error
            catcher('reconcile')(error, { resource: mlsResourceName })
          })
