...
module.exports = function(mlsSourceName, configBundle, eventEmitter, loggerArg) {
  ...
  async function downloadPurgeData() {
    ...
    for (const mlsResource of mlsResources) {
      ...
      const top = userConfig.sources[mlsSourceName].topForPurge
      if (!top) {
  ...
  async function downloadMlsResources() {
    ...
    _.set(internalConfig, ['sources', mlsSourceName, 'downloadSyncBatch'], downloadSyncBatch)
    await flushInternalConfig()
    ...
    for (const mlsResource of mlsResources) {
      logger.info({ resource: mlsResource }, 'Start downloading sync data for resource')
      const indexes = getIndexes(mlsResource)
      let timestamps
      ...
      while (url) {
        ...
        const dataFilePath = pathLib.resolve(mlsResourceDir, fileName)
        logger.debug({ resource: mlsResource, dataFilePath }, 'Saving sync data to file')
        await fsPromises.writeFile(dataFilePath, mlsDataString)
        ...
        // url = null
        const recordCount = mlsData.value.length
        logger.info({ resource: mlsResource, recordCount }, 'Payload contained number of sync records')
        count = mlsData['@odata.count']
      }

      downloadSyncBatch.mlsResourcesStatus.find(x => x.name === mlsResource).done = true
      await flushInternalConfig()
      logger.info({ resource: mlsResource }, 'Done downloading sync data for resource')
    }
    _.unset(internalConfig, ['sources', mlsSourceName, 'downloadSyncBatch'])
    await flushInternalConfig()
