...
module.exports = function(mlsSourceName, configBundle, eventEmitter, loggerArg) {
  ...
  function filterMlsResourcesFromBatch(mlsResources, batch) {
    ...
    return mlsResources.filter(mlsResourceName => {
      const batchResource = batch.mlsResourcesStatus.find(x => x.name === mlsResourceName)
  ...
  async function downloadPurgeData() {
    ...
    for (const mlsResourceName of mlsResources) {
      ...
      let url = new URL(userConfig.sources[mlsSourceName].getPurgeEndpoint(mlsResourceName))
  ...
  async function downloadMlsResources() {
    ...
    for (const mlsResourceName of mlsResources) {
      ...
      let url = userConfig.sources[mlsSourceName].getReplicationEndpoint(mlsResourceName)
