...
module.exports = function(mlsSourceName, configBundle, eventEmitter, loggerArg) {
  ...
  function getTimestampsFromMlsData(mlsData, indexes) {
    ...
    for (const record of resourceData) {
      for (keyName of keyNames) {
        if (record[keyName] && moment.utc(record[keyName]).isAfter(updateTimestamps[keyName])) {
  ...
  function makeUrl(mlsResourceObj, timestamps, top, batchAlreadyExisted, indexes, select, doExpand = true) {
    ...
    const timestampFieldNames = _.map(
      _.pickBy(indexes, (v, k) => v.isUpdateTimestamp),
      x => x.fields
    )
      .flat()
    if (select) {
      let _select = [...select]
      // We need certain fields, like the primary key and timestamp fields
      const primaryKeyFieldName = getPrimaryKeyField(mlsResourceObj.name, indexes)
      if (!_select.includes(primaryKeyFieldName)) {
        _select.unshift(primaryKeyFieldName)
      }
      const missingTimestampFieldNames = _.difference(timestampFieldNames, _select)
      if (missingTimestampFieldNames.length) {
        _select = [..._select, ...missingTimestampFieldNames];
      }
      urlWithTimestamps.searchParams.set('$select', _select.join(','))
    }
    if (mlsSourceUserConfig.useOrderBy) {
      urlWithTimestamps.searchParams.set('$orderby', timestampFieldNames.map(x => `${x} asc`)
        .join(','))
    }
    urlWithTimestamps.searchParams.set('$count', true)
    if (doExpand && mlsResourceObj.expand) {
  ...
  async function downloadAddFieldsData() {
    ...
    for (const mlsResourceObj of mlsResources) {
      ...
      const top = mlsSourceUserConfig.topForAddFields || mlsSourceUserConfig.top
      if (!top) {
        throw new Error('You must specify a "top" parameter in the config')
      }
  ...
  async function downloadMissingData() {
    ...
    for (let i = startIndex; i < mlsResources.length; i++) {
      ...
      while (missingIds.length) {
        ...
        if (!mlsSourceUserConfig.useOrderBy) {
          const sortedIds = _.sortBy(mlsData.value, x => x[primaryKey])
          lastIdDownloaded = sortedIds[sortedIds.length - 1][primaryKey]
        }
