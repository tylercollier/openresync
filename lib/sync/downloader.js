...
module.exports = function(mlsSourceName, configBundle, eventEmitter, loggerArg) {
  ...
  async function downloadMissingData() {
    /*
    * Starting after all IDs and timestamps have been downloaded.
    * Read in JSON files to get all IDs in MLS
    * Compare to all destinations, find out which IDs need to be synced
    *   * // This is TODO
    *   * Sort by ID and save to single JSON file
    * Download data, but don't name as _sync_.json or it could be picked up by sync cron. Use another name.
    *   * As we download, keep track of which ID we need to start the next batch with.
    *
    */
    ...
    for (let i = startIndex; i < mlsResources.length; i++) {
      ...
      const missingIds = await destinationManager.getMissingIds(mlsResourceName, dataInMls, mlsIndexes)
      const lastIdDownloaded = downloadMissingBatch.mlsResourcesStatus[i].lastIdDownloaded
      if (lastIdDownloaded) {
      ...
      while (missingIds.length) {
        ...
        downloadMissingBatch.mlsResourcesStatus[i].lastIdDownloaded = lastIdDownloaded
        await flushInternalConfig()
      }

      downloadMissingBatch.mlsResourcesStatus.find(x => x.name === mlsResourceName).done = true
      await flushInternalConfig()
